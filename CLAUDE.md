...
├── Plyne.xcodeproj/             # generated by `xcodegen generate` (NOT committed)
...
│   ├── PlyneCore/               # ✅ scaffolded — pure Swift, runs on Linux; domain types, timer FSM, math
│   ├── PlyneAnalytics/          # ✅ scaffolded — pure Swift; heatmap aggregation, trends, insight rules
│   ├── PlyneStorage/            # ✅ scaffolded — macOS-only; SwiftData schema and repositories
│   ├── PlyneTracker/            # ⏳ Phase 2 — macOS-only; NSWorkspace, AXObserver, AFK detection
│   ├── PlyneCalendar/           # ⏳ Phase 2 — macOS-only; EventKit access
│   └── PlyneFocus/              # ⏳ Phase 2 — macOS-only; Focus Filters, App Intents
├── .github/
│   └── workflows/
...
The Xcode project is generated from `project.yml` by [XcodeGen](https://github.com/yonaskolb/XcodeGen). The `.xcodeproj` is **not** committed — `project.yml` is the source of truth.

```bash
# One-time setup
brew install xcodegen

# Regenerate Plyne.xcodeproj from project.yml (run after pulling or editing project.yml)
xcodegen generate
...
# Full app build (macOS only — requires xcodegen generate first)
...
- **No `static var` in shared modules.** Mutable static state triggers strict-concurrency diagnostics and breaks testability. Use `actor` for shared mutable state, or push state into an injected dependency. `static let` on `Sendable` types is fine.
...
- **Commit author must always be `MolodykhV <molodykhvitaliy@gmail.com>`** (current `git config`). Earlier bootstrap commits `b8527ed` and `c8f502c` were made under an older identity (`molodykh_vitaliy <molodykh@realx.tech>`) — left intentionally (per ADR-style note); both belong to the same human and can be linked on GitHub via Settings → Emails. Do **not** rewrite that history.
